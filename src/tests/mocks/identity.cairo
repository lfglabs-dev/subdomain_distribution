...
trait IIdentity<TContractState> {
    ...
    fn transferFrom(
        ref self: TContractState, _from: ContractAddress, to: ContractAddress, token_id: u128,
    );
}
...
mod Identity {
    ...
    impl IdentityImpl of IIdentity<ContractState> {
        ...
        fn transferFrom(
            ref self: ContractState, _from: ContractAddress, to: ContractAddress, token_id: u128,
        ) {
            assert(!to.is_zero(), 'ERC721: invalid receiver');
            let owner = self.owner_by_id.read(token_id);

            self.owner_by_id.write(token_id, to);
        }
    }
}
